    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        if (isNetworkError && attempt < maxAttempts - 1) {
          ...
          console.warn(
            `[RPCManager] ${isRateLimit ? 'Rate limit' : 'RPC error'} on attempt ${attempt + 1}, ` +
            `waiting ${Math.round(delay / 1000)}s before rotating to ${this.getCurrentRpcUrl()}`,
          );
...
export function createRPCManager(): RPCManager {
  ...
  try {
    ...
    console.warn("[RPCManager] Error reading env variable, using defaults:", error);
  }
